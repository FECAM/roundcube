...
if [[ "$1" == apache2* ]] || [ "$1" == php-fpm ]; then
  ...
  if [ ! -e config/config.inc.php ]; then
    ...
    include(__DIR__ . '/config.docker.fecam.inc.php');
    ...
    echo "include(__DIR__ . '/config.docker.fecam.inc.php');" >> config/config.inc.php
  fi

  ROUNDCUBEMAIL_PLUGINS_PHP=`echo "${ROUNDCUBEMAIL_PLUGINS}" | sed -E "s/[, ]+/', '/g"`
  sed -i "s~%ROUNDCUBEMAIL_PASSWORD_DB_DSN~$ROUNDCUBEMAIL_PASSWORD_DB_DSN~" $PWD/config/config.docker.fecam.inc.php
  sed -i "s~%ROUNDCUBEMAIL_MANAGESIEVE_HOST~$ROUNDCUBEMAIL_MANAGESIEVE_HOST~" $PWD/config/config.docker.fecam.inc.php
